import {web3Sign, signEthers, signPolkadot, signBitcoin} from "../sharedFixtures";
...
describe("Test for signing: sign()", () => {
    ...
    it.each([
        ...
        [signPolkadot, "5F7MBfGdyTg5th5gzsWMUyaVBRUkhEZw5Q82rPrtSP1q9F3E", 0, "SR25519"],
        [signBitcoin, "1HZwtseQ9YoRteyAxzt6Zq43u3Re5JKPbk", 0, undefined]
    ])
    ('sign with %p', async (signFunc: Signer, address: string, verifierID: number, algorithm : string | undefined) =>{
        const token = await sign({
            "nonce" : 654321,
            "iat" : 1582062696,
            "exp" : 1782098690,
            "iss" : address
        }, signFunc, {verifierID, algorithm})
